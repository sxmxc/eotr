...
func _ready():
	...
	var player_starting_position = Vector2i(int(tilemap.map_width / 2.0), int(tilemap.map_height / 2.0))
...
func start_world() -> void:
	...
	get_tree().create_timer(1).timeout.connect(
		func(): 
			Events.world_message_requested.emit(WorldMessageData.new("The Obilisk has revealed itself"))
	)
