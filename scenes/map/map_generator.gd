...
func _setup_random_node_weights() -> void:
	...
	random_map_node_type_weights[Enums.MapNodeType.SHOP] = MONSTER_NODE_WEIGHT + REST_NODE_WEIGHT + SHOP_NODE_WEIGHT
...
func _set_map_node_randomly(nodes_to_set: MapNode) -> void:
	...
	while rest_below_4 or consecutive_rest or consecutive_shop or rest_on_13:
		...
		consecutive_rest = type_candidate == Enums.MapNodeType.REST and _map_node_has_parent_of_type(nodes_to_set, Enums.MapNodeType.REST)
		consecutive_shop = type_candidate == Enums.MapNodeType.SHOP and _map_node_has_parent_of_type(nodes_to_set, Enums.MapNodeType.SHOP)
	...
	if type_candidate == Enums.MapNodeType.MONSTER:
		...
		nodes_to_set.battle_stats = battle_stats_pool.get_random_battle_for_tier(tier_for_monster_nodes)
