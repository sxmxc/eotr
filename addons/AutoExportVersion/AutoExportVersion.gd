...
var BUILD_SETTING_NAME: String = "application/config/build"
...
func store_version(
	version: String, build: String, version_store_location := VersionStoreLocation.PROJECT_SETTING
) -> void:
	match version_store_location:
		...
		VersionStoreLocation.PROJECT_SETTING:
			store_version_as_project_setting(version, build)
...
func store_version_as_script(version: String) -> void:
	if version.is_empty():
		printerr(
			(
				"Cannot store version. "
				+ _EMPTY_VERSION_ERROR.format({"script_path": get_script().get_path()})
			)
		)
		return
...
func store_version_as_project_setting(version: String, build: String) -> void:
	if version.is_empty():
		printerr(
			(
				"Cannot store version. "
	...
	if not ProjectSettings.has_setting(BUILD_SETTING_NAME):
		ProjectSettings.set_initial_value(BUILD_SETTING_NAME, "Empty build")
		ProjectSettings.add_property_info(
			{
	...
	ProjectSettings.set_setting(BUILD_SETTING_NAME, build)
...
func _enter_tree() -> void:
	...
	ProjectSettings.add_property_info(
		{
			...
			"hint_string": "Script,Project Setting"
		}
	)
	...
	ProjectSettings.add_property_info(
		{"name": setting_name, "type": TYPE_STRING, "hint": PROPERTY_HINT_SAVE_FILE}
	)
	...
	if not ProjectSettings.has_setting(setting_name):
		...
		DirAccess.copy_absolute(
			"res://addons/AutoExportVersion/auto_export_version_config_file.gd", CONFIG_PATH
		)
	ProjectSettings.add_property_info(
		{"name": setting_name, "type": TYPE_STRING, "hint": PROPERTY_HINT_SAVE_FILE}
	)
...
func _sync_project_settings():
	...
	PROJECT_SETTING_NAME = ProjectSettings.get_setting(
		"addons/AutoExportVersion/version_setting_name"
	)

	var new_config_path: String = ProjectSettings.get_setting(
		"addons/AutoExportVersion/version_config_file"
	)
...
func _tool_menu_print_version() -> void:
	...
	var build: String = get_build(PackedStringArray(), true, "", 0)
	...
	store_version(version, build, STORE_LOCATION)
...
func get_build(features: PackedStringArray, is_debug: bool, path: String, flags: int) -> String:
	if not ResourceLoader.exists(CONFIG_PATH, "GDScript"):
		push_error("Build config file does not exist!")
		return ""

	var provider: RefCounted = load(CONFIG_PATH).new()
	return provider.get_build(features, is_debug, path, flags)
